...
#======================================================================================
pp_dir = f"/Users/cindyzhang/Documents/M2/Audiomotor_Piano/AM-EEG/data_preprocessed_{high_freq}Hz" #where the preprocessed files are
...
for folder in sorted(os.listdir(pp_dir)):
    ...
    data_path = glob.glob(os.path.join(sub_pp_dir, f'eeg_{task}_??.mat'))[0]
    ...
    t_keystrokes = clean_triggers(events_arr[events_arr[:, 2]==3]) 
        #trigger = 2 -> audio (example melodies to imitate)
        #trigger = 3 -> keystroke with audio feedback

    #take out the first 50 keystrokes, might not be clean
    t_keystrokes = t_keystrokes[50:-50]
    ...
    #plot evoked
    if plot:
        evoked.plot(titles = f'{task}-only keystrokes sub{subject_ID}')
    ...
    mne.write_evokeds(f'{evokeds_folder}/{task}_ERP_{subject_ID}.fif', evoked, overwrite = overwrite)
    epochs.save(f'{epochs_folder}/{task}_epochs_{subject_ID}.fif', overwrite = overwrite)
